    ...
    for c in cvals:
        ...
        for i in range(500):
            ...
            try:
                ...
                pass
        if randtimes_rw:
            rand_rw[(n,c)] = np.median(randtimes_rw)
        if randtimes_exp:
            rand_exp[(n,c)] = np.median(randtimes_exp)
        if randtimes_dfs:
            rand_dfs[(n,c)] = np.median(randtimes_dfs)
        
        if smallworldtimes_rw:
            smallworld_rw[(n,c)] = np.median(smallworldtimes_rw)
        if smallworldtimes_exp:
            smallworld_exp[(n,c)] = np.median(smallworldtimes_exp)
        if smallworldtimes_dfs:
            smallworld_dfs[(n,c)] = np.median(smallworldtimes_dfs)
        
        if scalefreetimes_rw:
            scalefree_rw[(n,c)] = np.median(scalefreetimes_rw)
        if scalefreetimes_exp:
            scalefree_exp[(n,c)] = np.median(scalefreetimes_exp)
        if scalefreetimes_dfs:
            scalefree_dfs[(n,c)] = np.median(scalefreetimes_dfs)
